...
function findChildren(basedir) {
  if(dirExist(basedir)) {
    ...
    let dirs = basedir ? getDirectories(basedir) : [];
    return dirs;
  }
  return null;
}
...
function initChildren(children, options, router, func) {
  if(children) for (var child of children) {
    initChild(child, options, router, func);
  }
}

function initChild(child, options, router, func) {
  let childOptions = _.cloneDeep(options);
  let parent = options.basedir || 'spec/';
  let childPath = '/' + removeParentPath(child, parent);
  ...
  if (!options.skipPlugins) {
    ...
    if(_.has(spec, 'children')) {
      _.forEach(spec.children, function(child, key){
        ...
        let cp = _.startsWith(childPath, 'spec/') ? childPath.substring(4) : childPath;
        let ppath = cp.substring(0, cp.lastIndexOf('/'));
...
async function addChildren(spec, children, parent) {
  if(children && parent) {
    ...
    for (var child of children) {
      let childName = removeParentPath(child, parent);
...
async function runPlugins(spec, options) {
  ...
  await parsePluginsLocator(spec, options, relativePluginsDir);
}

async function parsePluginsLocator(spec, options, content) {
  let dirs = _.split(content, '[;,\n]');
  for (var dir of dirs) {
    if(dir.indexOf('.') != -1) {
      //this is a file that contains list of dirs
      const pluginsFile = Path.join(process.cwd(), dir.trim());
      if (fs.existsSync(pluginsFile)) {
        let content = fs.readFileSync(pluginsFile, 'utf-8');
        await parsePluginsLocator(spec, options, content);
      }
    } else {
      await runPluginsInDir(spec, options, dir.trim());
    }
  }
}

async function runPluginsInDir(spec, options, relativePluginsDir) {
  ...
  plugins = sortPlugins(_.values(plugins));
...
function sortPlugins(plugins) {
  return _.sortBy(plugins, [function(o) { return o.order || 0; }]);
}
